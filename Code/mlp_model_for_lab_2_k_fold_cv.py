...
embedding_original_cols = [col for col in labeled_df.columns if "_original" in col and "Lab2" in col and not col.endswith("_valid")]
original_valid_cols = [col for col in labeled_df.columns if "_original_valid" in col and "Lab2" in col]
...
for train_idx, val_idx in kf.split(original_X):
    ...
    for epoch in range(epochs):
        ...
        vtrain_targets_original = y_scaler.inverse_transform(train_targets)
        ...
        train_mse = mean_squared_error(vtrain_targets_original, train_preds_original)
        train_r2 = r2_score(vtrain_targets_original, train_preds_original)
    ...
    fold_df["Lab2_Pred"] = val_preds_original.flatten()  # Saving final prediction
...
results_path = os.path.join(dataset_folder, "final_cv_lab2_results.txt")
with open(results_path, "w") as f:
    f.write(f"Average R2: {average_r2:.4f}\n")


oof_all_df = pd.concat(oof_df_list)
output_csv_path = os.path.join(dataset_folder, "oof_preds_lab2.csv")
