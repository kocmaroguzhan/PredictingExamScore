...
def multiply_question_answer_embeddings(original_df, question_embeddings_df, assesment_name):
    """
    Returns the element-wise product between question embeddings and student answer embeddings.
    ...
    Returns:
    - product_features: np.ndarray where each row is (question_embedding * student_answer_embedding)
    """
...
for assesment_name in assesment_names:
    ...
    question_answer_multiply_original_X = multiply_question_answer_embeddings(original_df, question_embeddings_df, assesment_name)
    question_answer_multiply_aug_X = multiply_question_answer_embeddings(augmented_df, question_embeddings_df, assesment_name)
...
for train_idx, val_idx in kf.split(question_answer_multiply_original_X):
    print(f"\n====== RidgeCV Fold {fold} ======")

    # Split features and labels
    X_train, X_val = question_answer_multiply_original_X[train_idx], question_answer_multiply_original_X[val_idx]
    ...
    # Train RidgeCV model
    ridge = RidgeCV(alphas=[0.01, 0.1, 1.0, 10.0, 100.0], scoring='neg_mean_squared_error', cv=3)
    ridge.fit(X_train_scaled, y_train_scaled)

    # Predict and evaluate
    val_preds = ridge.predict(X_val_scaled)
    val_preds_original = y_scaler.inverse_transform(val_preds.reshape(-1, 1)).flatten()
    val_mse = mean_squared_error(y_val, val_preds_original)
    val_r2 = r2_score(y_val, val_preds_original)
    print(f"🎯 RidgeCV Val MSE: {val_mse:.4f} | R²: {val_r2:.4f} | Alpha: {ridge.alpha_}")
    ...
    fold_df = pd.DataFrame({
        ...
        f"{assesment_name}_RidgeCV_Pred": val_preds
    })
...
# Summary
print(f"\n📊 RidgeCV Final CV Results for {assesment_name}")
print("Average R²:", np.mean(r2_scores))
print("Average MSE:", np.mean(mse_scores))

# Save predictions
#oof_all_df = pd.concat(oof_df_list)
#ridge_output_path = os.path.join(dataset_folder, f"ridgecv_oof_preds_{assesment_name}.csv")
