...
def generate_all_lab_embeddings():
    # Paths
    ...
    for file_name in os.listdir(labs_folder):
        ...
        lab_id = os.path.splitext(file_name)[0].split(".")[0]
        ...
        for student in tqdm(data["answers"], desc=f"Embedding {lab_id}"):
            ...
            if lab_id not in student_embeddings[student_id]:
                student_embeddings[student_id][lab_id] = {
                    "original": {},
                    "augmented": [],
                }
            # Save original
            student_embeddings[student_id][lab_id]["original"] = {
            ...
            for counter in range (augmentation_count):
                ...
                if was_augmented:
                    ...
                    if saved_augmented_count < number_of_augmented_code_saving_threshold:
                        ...
                        output_txt_path = os.path.join(augmentation_folder, f"{student_id}_{lab_id}_augmented_{counter}.txt")
                        with open(output_txt_path, "w", encoding="utf-8") as txt_file:
    ...
    # Fill missing lab data with zeros
    all_lab_ids = set()
    for lab_scores in student_embeddings.values():
        all_lab_ids.update(lab_scores.keys())
    ...
    rows = []
    for student_id, labs in student_embeddings.items():
    ...
    df_long["column_base"] = df_long["lab_id"] + "_" + df_long["type"]
    ...
    # Save final output
    output_csv_path = os.path.join(dataset_folder, "all_lab_embeddings.csv")
