...
embedding_original_cols = [col for col in labeled_df.columns if "_original" in col and "Lab1" in col and not col.endswith("_valid")]
original_valid_cols = [col for col in labeled_df.columns if "_original_valid" in col and "Lab1" in col]
...
for train_idx, val_idx in kf.split(original_X):
    ...
    fold_df["Lab1_Pred"] = val_preds_original.flatten()  # Saving final prediction
...
results_path = os.path.join(dataset_folder, "final_cv_lab1_results.txt")
with open(results_path, "w") as f:
    f.write(f"Average R2: {average_r2:.4f}\n")
    
oof_all_df = pd.concat(oof_df_list)
output_csv_path = os.path.join(dataset_folder, "oof_preds_lab1.csv")
